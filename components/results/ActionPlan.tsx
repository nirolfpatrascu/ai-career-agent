...
const TABS = [
  { key: 'thirtyDays' as const, label: '30-Day Quick Wins', icon: '⚡' },
  { key: 'ninetyDays' as const, label: '90-Day Skill Building', icon: '📚' },
  { key: 'twelveMonths' as const, label: '12-Month Trajectory', icon: '🚀' },
];
  ...
  return (
    <section>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {TABS.map((tab) => {
          const count = plan[tab.key].length;
          return (
            <button
              key={tab.key}
            ...
            >
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                activeTab === tab.key ? 'bg-primary/20' : 'bg-card-border'
              }`}>
                {count}
              </span>
            </button>
        ...
        {items.map((item: ActionItem, i: number) => (
          <div key={i} className="card hover:border-primary/20 transition-colors duration-200">
            <div className="flex items-start gap-4">
              {/* Step number */}
              <div className="w-8 h-8 rounded-lg bg-background flex items-center justify-center text-sm font-semibold text-text-secondary flex-shrink-0">
                {i + 1}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full border capitalize ${PRIORITY_STYLES[item.priority]}`}>
                    {item.priority}
                  </span>
