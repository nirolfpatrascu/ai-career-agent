  ...
  return (
    <section>
      <div className="flex items-center gap-3 mb-5">
        ...
        <h2 className="text-xl font-bold text-text-primary">{t('results.gaps.title')}</h2>
      </div>
      ...
      <div className="space-y-3">
        {gaps.map((gap, i) => {
          ...
          return (
            <div key={i} className={`card ${colors.border} border-l-4 cursor-pointer transition-all duration-200 hover:bg-card/80`} onClick={() => setExpandedIndex(isExpanded ? null : i)}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-text-secondary">{gap.timeToClose}</span>
                  </div>
                  <h3 className="font-semibold text-text-primary">{gap.skill}</h3>
                  <p className={`text-sm mt-1 ${colors.text}`}>{gap.impact}</p>
                </div>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`text-text-secondary flex-shrink-0 mt-1 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`}>
                ...
                <div className="mt-4 pt-4 border-t border-card-border space-y-4 animate-fade-in">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="bg-background rounded-lg p-3">
                      <p className="text-xs text-text-secondary mb-1">{t('results.gaps.currentLevel')}</p>
                      <p className="text-sm text-text-primary">{gap.currentLevel}</p>
                    </div>
                    <div className="bg-background rounded-lg p-3">
                      <p className="text-xs text-text-secondary mb-1">{t('results.gaps.requiredLevel')}</p>
                      <p className="text-sm text-text-primary">{gap.requiredLevel}</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-xs text-text-secondary mb-1">{t('results.gaps.closingPlan')}</p>
                    <p className="text-sm text-text-primary leading-relaxed">{gap.closingPlan}</p>
                  </div>
                  {gap.resources.length > 0 && (
                    <div>
                      <p className="text-xs text-text-secondary mb-2">{t('results.gaps.resources')}</p>
