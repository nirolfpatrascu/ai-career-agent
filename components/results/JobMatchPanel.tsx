  ...
  return (
    <section>
      <div className="flex items-center gap-3 mb-5">
        ...
        <h2 className="text-xl font-bold text-text-primary">{t('results.jobMatch.title')}</h2>
      </div>

      <div className="space-y-4">
        <div className="card text-center py-8">
          <div className="text-5xl font-bold mb-2" style={{ color }}>{match.matchScore}%</div>
          <p className="text-text-secondary">{t('results.jobMatch.matchScore')}</p>
        </div>
          ...
          <div className="card">
            ...
            <div className="flex flex-wrap gap-1.5">
              {match.matchingSkills.map((skill, i) => (
                <span key={i} className="text-xs px-2.5 py-1 rounded-md bg-success/10 text-success border border-success/20">{skill}</span>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="flex flex-wrap gap-1.5">
              {match.missingSkills.map((skill, i) => (
                <span key={i} className="text-xs px-2.5 py-1 rounded-md bg-danger/10 text-danger border border-danger/20">{skill}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        {match.cvSuggestions.length > 0 && (
          <div>
            ...
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`transition-transform ${showSuggestions ? 'rotate-180' : ''}`}>
              ...
              <div className="space-y-3 animate-fade-in">
                {match.cvSuggestions.map((suggestion, i) => (
                  <div key={i} className="card">
                    <p className="text-xs font-medium text-primary mb-3">{suggestion.section}</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                      <div className="bg-danger/5 rounded-lg p-3 border border-danger/10">
                        <p className="text-xs text-danger font-medium mb-1">{t('results.jobMatch.current')}</p>
                      ...
                      <div className="bg-success/5 rounded-lg p-3 border border-success/10">
                        <p className="text-xs text-success font-medium mb-1">{t('results.jobMatch.suggested')}</p>
