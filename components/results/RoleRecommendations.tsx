  ...
  return (
    <section>
      <div className="flex items-center gap-3 mb-5">
        ...
        <h2 className="text-xl font-bold text-text-primary">Recommended Roles</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {roles.map((role, i) => {
          ...
          return (
            <div
              key={i}
              className={`card relative overflow-hidden ${i === 0 ? 'ring-1 ring-primary/30' : ''}`}
            >
              {/* Best fit badge */}
              {i === 0 && (
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-xs text-text-secondary">Fit Score</p>
                  <p className="text-xs" style={{ color }}>out of 10</p>
                </div>
              </div>
              ...
              <div className="bg-background rounded-lg p-3 mb-4">
                ...
                <p className="text-lg font-semibold text-success">
                  {formatCurrency(role.salaryRange.low, role.salaryRange.currency)} –{' '}
                  {formatCurrency(role.salaryRange.high, role.salaryRange.currency)}
                </p>
                <p className="text-xs text-text-secondary mt-0.5">
                  Mid: {formatCurrency(role.salaryRange.mid, role.salaryRange.currency)}
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {role.exampleCompanies.map((company, ci) => (
                  <span
                    key={ci}
                    className="text-xs px-2.5 py-1 rounded-md bg-card-border/50 text-text-secondary"
                  >
                    {company}
                  </span>
                ))}
