...
function FileCard({
  ...
  return (
    <div className={`rounded-xl border ${colors.border} bg-white/[0.02] p-4 flex items-center gap-3 transition-all duration-200`}>
      ...
      <button onClick={onRemove} className="text-text-tertiary hover:text-danger transition-colors p-1 rounded-lg hover:bg-danger/[0.06]" aria-label="Remove file">
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <div className="w-5 h-5 rounded flex items-center justify-center bg-[#0A66C2]/[0.1]">
        ...
        {detecting && (
          <div className="mt-2.5 flex items-center gap-2 px-3.5 py-2.5 bg-primary/[0.04] border border-primary/10 rounded-xl">
            ...
            <span className="text-xs text-primary font-medium">{t('docUpload.detecting')}</span>
          </div>
        )}
        ...
        {autoDetected && !detecting && (
          <div className="mt-2.5 flex items-start gap-2 px-3.5 py-2.5 bg-success/[0.04] border border-success/15 rounded-xl">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-text-tertiary mt-0.5 truncate">{autoDetected}</p>
            </div>
            <button onClick={onDismissDetection} className="text-text-tertiary hover:text-text-secondary flex-shrink-0 p-0.5" aria-label="Dismiss">
        ...
        {!linkedInFile && (
          ...
          >
            ...
            <span className="text-xs text-text-tertiary">{t('docUpload.guideQuestion')}</span>
            <svg
              width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
              className={`text-text-tertiary ml-auto transition-transform duration-300 ${showGuide ? 'rotate-180' : ''}`}
            >
      ...
      {/* Validation hint */}
      {!hasAnyFile && (
        <p className="text-xs text-text-tertiary text-center pt-1">{t('docUpload.uploadHint')}</p>
      )}
    </div>
  );
}